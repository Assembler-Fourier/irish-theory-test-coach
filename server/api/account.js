...
import { buildAccountSnapshot } from "../../lib/account-data.js";
import {
  getAuthServerEnv,
  safeErrorSummary,
  sendSafeConfigError,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const account = await buildAccountSnapshot(env.databaseUrl, session);
    return res.status(200).json({ ok: true, ...account });
  } catch (error) {
    console.error("Could not load account snapshot", safeErrorSummary(error));
