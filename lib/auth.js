...
import { hashIdentifier } from "./rate-limit.js";
...
export async function readJsonBody(req, options = {}) {
  const maxBytes = Number(options.maxBytes || process.env.MAX_JSON_BODY_BYTES || 32_768);
  if (req.body && typeof req.body === "object") return req.body;
  if (typeof req.body === "string") {
    if (Buffer.byteLength(req.body, "utf8") > maxBytes) throwHttpError(413, "request_body_too_large");
    try {
      return req.body ? JSON.parse(req.body) : {};
    } catch {
      throwHttpError(400, "malformed_json");
    }
  }
  ...
  for await (const chunk of req) {
    ...
    if (totalBytes > maxBytes) throwHttpError(413, "request_body_too_large");
    chunks.push(buffer);
  }
...
export async function deliverMagicLink({ email, link, env }) {
  ...
  if (!configured) {
    if (!env.isProduction && link) {
      console.info("Magic login link was created but not logged because no email provider is configured.");
...
function clientIp(req) {
  ...
  return raw ? `iphash_${hashIdentifier(raw)}` : "";
}
...
function throwHttpError(statusCode, code) {
