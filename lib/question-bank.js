...
import { annotateQuestionsWithQuality } from "./content-quality.js";
...
export function getPrivateQuestionBank(env = process.env) {
  ...
  const questions = annotateQuestionsWithQuality((Array.isArray(raw) ? raw : raw.questions)
...
export function selectQuestionsForMode(questions, options = {}) {
  ...
  return selectDistinctVariants(pool, limit);
}
...
function buildMockQuestions(pool, options) {
  ...
  return selectDistinctVariants(shuffle(categoryPool.length >= 40 ? categoryPool : pool)
    .sort((a, b) => b.priorityScore - a.priorityScore || a.id - b.id)
    .slice(0, 60)
    .sort(() => Math.random() - 0.5), 40);
}

function selectDistinctVariants(pool, limit) {
  const selected = [];
  const seenGroups = new Set();
  const deferred = [];

  for (const question of pool) {
    const groupId = question.variantGroupId || `vg_single_${question.id}`;
