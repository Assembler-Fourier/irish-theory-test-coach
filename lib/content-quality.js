...
function analyzeDuplicates(questions) {
  ...
  return { duplicateGroups, conflictingAnswerGroups, questionIndex };
}
...
function formatDuplicateGroup(group, questions) {
  ...
  return {
    ...
    reviewStatus: correctAnswers.length > 1 ? "conflict_queue" : "needs_review",
    conflictingCorrectAnswers: correctAnswers.length > 1,
    ...
    legitimateScenarioVariant: false,
  };
}
...
function buildEditorialBacklog({ duplicateAnalysis, lintFindings }) {
  const duplicateRows = duplicateAnalysis.duplicateGroups.map((group) => ({
    itemType: group.conflictingCorrectAnswers ? "conflict" : "duplicate",
    priority: group.conflictingCorrectAnswers ? "critical" : "medium",
    groupId: group.variantGroupId,
    questionIds: group.questionIds,
    reason: group.duplicateReason,
    reviewStatus: group.reviewStatus,
    message: group.conflictingCorrectAnswers
      ? "Repeated stem has conflicting correct answers; editorial decision required."
