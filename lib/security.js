...
export function verifyStateChangingRequest(req, env, options = {}) {
  ...
  const isProduction = Boolean(env?.isProduction || process.env.VERCEL_ENV === "production" || process.env.NODE_ENV === "production");
  ...
  const originOk = suppliedOrigin === expected || (!isProduction && isLocalOrigin(suppliedOrigin));
  const hostOk = !hostOrigin || hostOrigin === expected || (!isProduction && isLocalOrigin(hostOrigin));
  if (!originOk || !hostOk) {
